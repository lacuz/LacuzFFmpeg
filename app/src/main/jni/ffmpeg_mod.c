...
#define CB_CLASS "com/lacuz/ffmpeg/FFmpegCmd"
static void callback_java_progress(double);
static void callback_java_error(jstring msg);
static void callback_java_finshed();

#define CB_METHOD_PROGRESS "onProgress"
#define CB_METHOD_FINISHED "onFinish"
#define CB_METHOD_ERROR "onError"
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            av_bprintf(&buf_script, "stream_%d_%d_q=%.1f\n",
                       ost->file_index, ost->index, q);
            if (is_last_report)
                snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "Last ");
            if (qp_hist) {
                int j;
                int qp = lrintf(q);
    ...
    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
             ...
             (100 * us) / AV_TIME_BASE);
    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
                                     "\noc2->duration=%lld,pts=%d,has trade=%lf\n",oc2->duration,pts,((double)pts/oc2->duration)*100);

    callback_java_progress(((double)pts/oc2->duration)*100);
...
jmethodID mSendStr;
int status;
static void callback_java_progress(double progress)
{
	if(!jcls || status==0){
		jcls = (*envt)->FindClass(envt,CB_CLASS);
	}
	if(!jcls ){
	    av_log(NULL, AV_LOG_WARNING, "Unable to find class:%s",CB_CLASS);
	    return ;
	}

    if(!mSendStr|| status==0){
//    	mSendStr = (*envt)->GetStaticMethodID(envt,jcls,CB_METHOD_PROGRESS,"(I)V");
    	mSendStr = (*envt)->GetMethodID(envt,jcls,CB_METHOD_PROGRESS,"(I)V");
    }
    if(mSendStr|| status==0){
//    	(*envt)->CallStaticVoidMethod(envt,jcls,mSendStr,(jint)progress);
    	(*envt)->CallVoidMethod(envt,thisObj,mSendStr,(jint)progress);
    }else{
        av_log(NULL, AV_LOG_WARNING, "Unable to find method:%s",CB_METHOD_PROGRESS);
    }
    status=1;
...
//	jmethodID mfinsh = (*envt)->GetStaticMethodID(envt,jcls,CB_METHOD_FINISHED,"()V");
	jmethodID mfinsh = (*envt)->GetMethodID(envt,jcls,CB_METHOD_FINISHED,"()V");

    if(mSendStr){
//    	(*envt)->CallStaticVoidMethod(envt,jniutils,mfinsh);
    	(*envt)->CallVoidMethod(envt,thisObj,mfinsh);
    }else{
        av_log(NULL, AV_LOG_WARNING, "Unable to find method:%s",CB_METHOD_FINISHED);
    }
...
static void callback_java_error(jstring  msg){
	...
	jmethodID mError = (*envt)->GetMethodID(envt,jcls,CB_METHOD_ERROR,"(Ljava/lang/String;)V");

    if(mError){
    	(*envt)->CallVoidMethod(envt,thisObj,mError,msg);
    }else{
        av_log(NULL, AV_LOG_WARNING, "Unable to find method:%s",CB_METHOD_FINISHED);
    }
}

void com_java_cancel(){
	exit_program(1);
}
...
int ffmpegmain( JNIEnv * env,jobject thiz,int argc, char **argv)
{
    ...
    if (nb_output_files <= 0 && nb_input_files == 0) {
	    ...
	    callback_java_error((*env)->NewStringUTF(env, "找不到文件"));
        ffmpeg_cleanup(1);
        return 1;
    }
    ...
    if (nb_output_files <= 0) {
	    ...
	    callback_java_error((*env)->NewStringUTF(env, "找不到输出文件"));
        ffmpeg_cleanup(1);
        return 1;
    }
...
//     }
    ...
    if (transcode() < 0)
    {
	    callback_java_error((*env)->NewStringUTF(env, "转码失败"));
    	ffmpeg_cleanup(1);
    	return 1;
    }
    ti = getutime() - ti;
     av_log(NULL, AV_LOG_FATAL, "Transcode has Finished\n");
     callback_java_finshed();
  // if (do_benchmark) {
